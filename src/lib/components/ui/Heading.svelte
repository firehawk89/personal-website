<script lang="ts">
  ...
  const tag = `h${level}` satisfies keyof HTMLElementTagNameMap
  const headingClass = cn(headingVariants({ position, size, variant }), className)
</script>

<svelte:element this={tag} class={headingClass} {...props}>
  {@render children?.()}
</svelte:element>
